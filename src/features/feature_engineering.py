...
#     df["num_med_changes"] = df[meds].isin(["Up", "Down"]).sum(axis=1)
#     df["num_med_active"] = df[meds].ne("No").sum(axis=1)
...
#     # ✅ CREATE TARGET HERE (FIXED!)
#     target_cfg = params_fe["target"]
#     df[target_cfg["column_name"]] = df[target_cfg["source_column"]].map(
#         target_cfg["mapping"]
#     )

#     print(f"✓ Target '{target_cfg['column_name']}' created")
#     print(f"  Distribution: {df[target_cfg['column_name']].value_counts().to_dict()}")
...
#     # ✅ DROP COLUMNS (including raw target 'readmitted')
#     drop_cols = params_fe["feature_selection"]["drop_column"]
#     df = df.drop(columns=drop_cols, errors="ignore")
...
#     feature_info = {
#         'all_columns': df.columns.tolist(),
#         'target_column': target_cfg["column_name"]
#     }
